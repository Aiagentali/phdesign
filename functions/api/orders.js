...
// GET /api/orders - list my orders, POST /api/orders - create
...
export async function onRequestPost({ request, env }) {
  ...
  const user = await getUserFromRequest(request, env);
  if (!user) return json({error:'وارد نشده‌اید - لطفاً وارد شوید'},401,corsHeaders(request));
  ...
  // Load pricing from settings for validation
  ...
  await exec(env.DB, 'INSERT INTO orders (id,user_id,type,type_name,base_price,addons_json,addons_price,total_price,days,status,customer_note,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
    [id, user.id, type, typeInfo.name, typeInfo.price, JSON.stringify(validAddons), addonsPrice, total, days, 'pending', note, now, now]);
  await exec(env.DB, 'INSERT INTO audit_logs (id,actor_id,action,target_type,target_id,meta_json,created_at) VALUES (?,?,?,?,?,?,?)',
    [uuid(), user.id, 'create_order','order',id, JSON.stringify({type,total}), now]);

  return json({ok:true, order:{id, type, type_name:typeInfo.name, total_price:total, days, status:'pending'}},200,corsHeaders(request));
}
